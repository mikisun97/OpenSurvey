...
COMMENT ON TABLE COMTNEMPLYRSCRTYESTBS IS '업무사용자보안설정';
...
CREATE TABLE IF NOT EXISTS COMTNMENUINFO (
    MENU_NO BIGSERIAL PRIMARY KEY,                       -- 메뉴번호
    MENU_ID VARCHAR(20) NOT NULL UNIQUE,                 -- 메뉴ID
    MENU_NM VARCHAR(60) NOT NULL,                        -- 메뉴명
    UPPER_MENU_ID VARCHAR(20),                           -- 상위메뉴ID
    MENU_ORDR INTEGER NOT NULL,                          -- 메뉴순서
    MENU_DC VARCHAR(500),                                -- 메뉴설명
    RELATE_IMAGE_PATH VARCHAR(100),                      -- 관련이미지경로
    RELATE_IMAGE_NM VARCHAR(60),                         -- 관련이미지명
...
CREATE TABLE IF NOT EXISTS COMTNPROGRMLIST (
    PROGRM_FILE_NM VARCHAR(60) PRIMARY KEY,              -- 프로그램파일명
    PROGRM_STRE_PATH VARCHAR(100) NOT NULL,              -- 프로그램저장경로
    PROGRM_KOREAN_NM VARCHAR(60),                        -- 프로그램한글명
    PROGRM_DC VARCHAR(200),                              -- 프로그램설명
...
CREATE TABLE IF NOT EXISTS COMTNMENUCREATDTLS (
    MENU_NO BIGINT NOT NULL,                             -- 메뉴번호
    AUTHOR_CODE VARCHAR(30) NOT NULL,                    -- 권한코드
    MAPNG_CREAT_ID VARCHAR(30),                          -- 매핑생성ID
    
    PRIMARY KEY (MENU_NO, AUTHOR_CODE),
    CONSTRAINT FK_COMTNMENUCREATDTLS_MENU FOREIGN KEY (MENU_NO) 
        REFERENCES COMTNMENUINFO(MENU_NO) ON DELETE CASCADE,
    CONSTRAINT FK_COMTNMENUCREATDTLS_AUTHOR FOREIGN KEY (AUTHOR_CODE) 
        REFERENCES COMTNAUTHORINFO(AUTHOR_CODE) ON DELETE CASCADE
);

COMMENT ON TABLE COMTNMENUINFO IS '메뉴정보';
COMMENT ON TABLE COMTNPROGRMLIST IS '프로그램목록';
COMMENT ON TABLE COMTNMENUCREATDTLS IS '메뉴생성상세';
...
COMMENT ON TABLE COMTNSURVEY IS '설문조사';
...
-- 메뉴 인덱스
CREATE INDEX IF NOT EXISTS IDX_COMTNMENUINFO_UPPER_MENU ON COMTNMENUINFO(UPPER_MENU_ID);
CREATE INDEX IF NOT EXISTS IDX_COMTNMENUINFO_ORDR ON COMTNMENUINFO(MENU_ORDR);
...
VALUES 
('ROLE_ADMIN', '시스템관리자', '시스템 전체 관리 권한', TO_CHAR(CURRENT_DATE, 'YYYYMMDD'), 'system'),
('ROLE_USER', '일반사용자', '일반 사용자 권한', TO_CHAR(CURRENT_DATE, 'YYYYMMDD'), 'system'),
('ROLE_ANONYMOUS', '익명사용자', '비회원 권한', TO_CHAR(CURRENT_DATE, 'YYYYMMDD'), 'system')
...
VALUES 
('GROUP_00000000000001', '전체관리자', TO_CHAR(CURRENT_DATE, 'YYYYMMDD'), '시스템 전체 관리자 그룹', 'system'),
('GROUP_00000000000002', '부분관리자', TO_CHAR(CURRENT_DATE, 'YYYYMMDD'), '부분 관리자 그룹', 'system'),
('GROUP_00000000000003', '일반사용자', TO_CHAR(CURRENT_DATE, 'YYYYMMDD'), '일반 사용자 그룹', 'system')
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE '전자정부 표준 테이블 생성 완료!';
    RAISE NOTICE '========================================';
    ...
    RAISE NOTICE '6. 메뉴 관리: COMTNMENUINFO, COMTNPROGRMLIST, COMTNMENUCREATDTLS';
    RAISE NOTICE '7. 설문조사: COMTNSURVEY';
    RAISE NOTICE '========================================';
