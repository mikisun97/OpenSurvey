-- =====================================================
-- 전자정부 표준 게시판 구조로 마이그레이션
-- OpenSurvey Project - Database Migration
-- =====================================================
...
-- 기존 테이블들을 역순으로 삭제 (외래키 때문에)
DROP TABLE IF EXISTS BOARD_COMMENT CASCADE;
DROP TABLE IF EXISTS BOARD_FILE CASCADE;
DROP TABLE IF EXISTS BOARD CASCADE;

-- 관련 시퀀스도 삭제 (있다면)
DROP SEQUENCE IF EXISTS board_board_seq_seq CASCADE;
DROP SEQUENCE IF EXISTS board_file_file_seq_seq CASCADE;
DROP SEQUENCE IF EXISTS board_comment_comment_seq_seq CASCADE;

-- 기존 게시판 타입 공통코드 삭제 (있다면)
DELETE FROM COMTCCMMNDETAILCODE WHERE CODE_ID IN ('BOARD_TYPE', 'BOARD_DETAIL_TYPE');
DELETE FROM COMTCCMMNCODE WHERE CODE_ID IN ('BOARD_TYPE', 'BOARD_DETAIL_TYPE');

-- =====================================================
-- 2. 전자정부 표준 게시판 테이블 생성
-- =====================================================

-- 게시판 마스터 테이블 (COMTNBBSMST)
...
BEGIN
    RAISE NOTICE '========================================';
    RAISE NOTICE '전자정부 표준 게시판 마이그레이션 완료!';
    RAISE NOTICE '========================================';
    RAISE NOTICE '삭제된 기존 테이블:';
    RAISE NOTICE '- BOARD (기존)';
    RAISE NOTICE '- BOARD_FILE (기존)';
    RAISE NOTICE '- BOARD_COMMENT (기존)';
    RAISE NOTICE '========================================';
    RAISE NOTICE '생성된 신규 테이블:';
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '다음 단계: 백엔드 VO/Service/Controller 수정';
    RAISE NOTICE '========================================';
END $$; 