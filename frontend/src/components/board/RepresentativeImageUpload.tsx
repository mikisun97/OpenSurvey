...
interface RepresentativeImageData {
  id?: string;
  url?: string;
  name?: string;
  file?: File;
...
interface RepresentativeImageUploadProps {
  // 게시판 설정 정보
  boardInfo?: {
    representImageUseAt?: string;
    representImageWidth?: number;
    representImageHeight?: number;
  } | null;
  
  // 현재 이미지 데이터 (수정 시 기존 이미지)
  currentImage?: RepresentativeImageData | null;
...
export default function RepresentativeImageUpload({
  ...
  // 게시판에서 대표이미지를 사용하지 않는 경우 렌더링하지 않음
  if (boardInfo?.representImageUseAt !== 'Y') {
    return null;
  }
